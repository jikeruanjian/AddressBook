...
import com.example.ui.MainActivity.MainActivityListAdapter.ViewHolder;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				switch (msg.what) {
				case 0: { // 重新加载数据
					list.setAdapter(new MainActivityListAdapter(
							context, dataList));
					hideProgressDialog();
					break;
				}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (dataList.size() == 0) {
					...
					// 初始化数据
//					dataList = new ToolPDBll().getAllCanStockCountSafeTools("");
				}
				hideProgressDialog();
